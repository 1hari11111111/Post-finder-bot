...
async def get_groups():
    count = await grp_col.count_documents({})
    cursor = grp_col.find({})
    groups_list = await cursor.to_list(length=int(count))
    return count, groups_list
...
async def get_users():
    ...
    users_list = await cursor.to_list(length=int(count))
...
async def search_imdb(query):
    try:
        ...
        for movie in movies:
            ...
            movie_list.append({"title": title, "year": year, "id": movie.movieID})
        return movie_list
...
async def force_sub(bot, message):
    ...
    try:
        ...
        await message.reply(
            f"⚠ Dear User {message.from_user.mention}!\n\nTo send messages in this group, you need to join our channel first.",
